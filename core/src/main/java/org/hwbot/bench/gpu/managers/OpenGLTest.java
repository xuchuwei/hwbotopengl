...
public class OpenGLTest extends BenchmarkState {
    ...
    public void draw() {
        ...
        beginRender(true);
        if (!loading) {
            quad.render(atmosphereShader, GL10.GL_TRIANGLE_FAN);
            renderWorld();
            ...
            GL20 gl = Gdx.gl20;
            Gdx.app.log("OpenGL", "OGL renderer (best guess videocard, okay for nvidia, not for ati): " + gl.glGetString(GL10.GL_RENDERER));
            Gdx.app.log("OpenGL", "OGL vendor (manufacturer): " + gl.glGetString(GL10.GL_VENDOR));
            // Gdx.app.log("AndroidGraphics", "OGL version (driver?): " + gl.glGetString(GL10.GL_VERSION));
            // Gdx.app.log("AndroidGraphics", "OGL extensions (supported features): " + gl.glGetString(GL10.GL_EXTENSIONS));
            // Gdx.app.log("AndroidGraphics", "OGL 0x21A4: " + gl.glGetString(0x21A4));
            performance.append("Loading using " + gl.glGetString(GL10.GL_VENDOR) + ", " + gl.glGetString(GL10.GL_VERSION) + ", "
                    + gl.glGetString(GL10.GL_RENDERER) + "...");

            String feats[] = gl.glGetString(GL10.GL_EXTENSIONS).split(" ");
